...
namespace Vaje2
{
    class Vaje2
    {
        static void Main(String[] args)
        {
            ...
            // kot 
            double cosA = (Math.Pow(st2, 2) + Math.Pow(hipotenuza, 2) - Math.Pow(st1, 2)) / (2 * st2 * hipotenuza); //uporaba kosinusnega izreka
            double kotA = Math.Acos(cosA);
            double alfa = kotA * (180.0 / Math.PI); //Pretvorim iz radiane v stopinje
            Console.WriteLine("Kot alfa je " + Math.Round(alfa,2));
            
            double cosB = (Math.Pow(st1, 2) + Math.Pow(hipotenuza, 2) - Math.Pow(st2, 2)) / (2 * st1 * hipotenuza);
            double kotB = Math.Acos(cosB);
            double beta = kotB * (180.0 / Math.PI);
            Console.WriteLine("Kot beta je " + Math.Round(beta, 2));
            
            double cosC = (Math.Pow(st1, 2) + Math.Pow(st2, 2) - Math.Pow(hipotenuza, 2)) / (2 * st1 * st2);
            double kotC = Math.Acos(cosC);
            double gama = kotC * (180.0 / Math.PI);
            Console.WriteLine("Kot gama je " + Math.Round(gama, 2));
            
            // polmer včrtanega kroga 
            double s = (st1 + st2 + hipotenuza) / 2; // polovica obsega
            double ploscina = Math.Sqrt(s * (s - st1) * (s - st2) * (s - hipotenuza)); //Heronova formula za Ploščino
            double radijVcrtanega = Math.Round(ploscina / s,5); //radij za včrtani krog
            Console.WriteLine("Polmer včrtanega kroga je " + radijVcrtanega); 
            
            
            // polmer očrtanega kroga todo
            double radijOcrtanega = Math.Round((st1 * st2 * hipotenuza) / ploscina,5); // radij  za očrtani krog

            Console.WriteLine("Polmer očrtanega kroga je " + radijOcrtanega);

            // ploščina
            Console.WriteLine("Ploščina trikotnika je " + Math.Round(ploscina,5)); //uporabimo predhodno ustvarjeno formulo

            // višina na hipotenuzo v pravokotnem trikotniku
            double visina = Math.Round((st1 * st2) / hipotenuza,5); //izpeljemo heronovo formulo
            Console.WriteLine("Višina na hipotenuzo je " + visina);

            // ali je enakokraki
            if (st1 == st2 || st1 == hipotenuza || st2 == hipotenuza)
            {
            ...
            // kvadratni koren manjsega števila

            double kvadratniKoren = Math.Min(stevilo1, stevilo2);
            kvadratniKoren = Math.Round(Math.Sqrt(kvadratniKoren),5); // za malce lepši izpis
            Console.WriteLine("Kvadratni koren manjšega števila je " + kvadratniKoren);
            
            // Povprečna vrednost obeh števil

            double povp = (stevilo1 + stevilo2) / 2.0;
            Console.WriteLine("Povprečje dveh števil je " + povp);
            
            // vrednost potence, ki ima za osnovo st1, za eskponent pa st2
            double potenca = Math.Pow(stevilo1, stevilo2);
            Console.WriteLine("Potenca števila " + stevilo1 + " z osnovo " + stevilo2 + " je " + potenca);
